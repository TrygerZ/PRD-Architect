      ...
      {versions.length > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 bg-cyber-surface/60 border border-cyber-border/40 p-4 rounded-xl no-print">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 border-r border-cyber-border/50 pr-4">
              <span className="text-cyber-text-dim text-sm font-medium">
                Version:
              </span>
              <select
                ...
                className="bg-black/50 border border-cyber-border/60 rounded px-2 py-1 text-sm text-cyber-text font-mono focus:outline-none focus:border-cyber-accent min-w-32 max-w-48"
              >
                {versions.map((v, i) => (
            ...
            {versions.find((v) => v.id === activeVersionId)
              ...
              <div className="flex items-center gap-2 px-2 text-xs font-mono text-cyber-accent">
                <span className="px-1.5 py-0.5 bg-cyber-accent/10 border border-cyber-accent/30 rounded">
                  {
                    versions.find((v) => v.id === activeVersionId)
                      ?.referencedFilesCount
                  }{" "}
          ...
          <div className="flex items-center gap-4">
            <div className="text-sm text-cyber-text-dim flex items-center gap-2">
              ...
              <span>
                {totalComments} {language === "en" ? "Comments" : "Komentar"}
              </span>
            </div>
...
function SheetSection({
  ...
  return (
    ...
    >
      ...
      <div
        className="flex-grow min-w-0 p-4 sm:p-6 lg:border-r border-cyber-border/40 prose prose-invert prose-cyber max-w-none 
      ...
      >
        <ReactMarkdown
          ...
          components={{
            ...
            thead: ({ node, ...props }) => (
              <thead
                className="bg-cyber-surface/90 text-cyber-accent font-mono border-b border-cyber-border print:bg-gray-100 print:text-black print:border-gray-300"
                {...props}
              />
            ),
            th: ({ node, ...props }) => (
              <th
                className="px-5 py-3.5 font-semibold tracking-wide whitespace-nowrap"
                {...props}
              />
            ),
            tbody: ({ node, ...props }) => (
              <tbody
                className="divide-y divide-cyber-border/40 print:divide-gray-200"
                {...props}
              />
            ),
            tr: ({ node, ...props }) => (
              <tr
                className="hover:bg-cyber-accent/5 transition-colors duration-200"
                {...props}
              />
            ),
            td: ({ node, ...props }) => (
              <td
                className="px-5 py-4 align-top leading-relaxed text-cyber-text-dim print:text-black max-w-xs break-words"
                {...props}
              />
            ),
            pre: ({ node, children, ...props }) => (
              <div className="relative my-6 rounded-lg border border-cyber-border/40 bg-black overflow-hidden print:bg-gray-50 print:border-gray-300">
                ...
                <pre
                  className="p-4 pt-12 overflow-x-auto m-0 bg-transparent text-sm font-mono text-cyber-text-dim print:text-black"
                  {...props}
                >
                  {children}
                </pre>
            ...
            code: ({ node, className, children, ...props }: any) => {
              ...
              if (isInline) {
                return (
                  <code
                    className="px-1.5 py-0.5 mx-0.5 rounded bg-cyber-surface/80 text-cyber-accent font-mono text-[0.85em] border border-cyber-border/50 print:bg-gray-100 print:text-black print:border-gray-200"
                    {...props}
                  >
                    {children}
                  </code>
      ...
      <div className="w-full lg:w-72 lg:shrink-0 bg-black/20 p-4 relative no-print flex flex-col group/feedback">
        <label className="text-xs font-mono text-cyber-text-dim uppercase tracking-wider mb-3 flex items-center gap-2">
          ...
          {language === "en" ? "Revision / Feedback" : "Revisi / Feedback"}
        </label>
        ...
        {isEditing ? (
          <div className="flex flex-col gap-2 relative z-20">
            <textarea
              ...
              placeholder={
                language === "en"
                  ? "Add feedback or revision for this section..."
                  : "Berikan feedback atau revisi untuk bagian ini..."
              }
            />
          ...
          >
            {comment ? (
              ...
              <div className="p-3 text-sm text-cyber-text-dim/50 flex-grow border border-dashed border-cyber-border/30 rounded-lg hover:border-cyber-border/70 hover:text-cyber-text-dim transition-all">
                {language === "en"
                  ? "Click to add revision notes for this section..."
            ...
            {comment && !isGenerating && (
              <div className="absolute top-2 right-2 opacity-0 group-hover/feedback:opacity-100 transition-opacity">
                <button className="text-xs bg-black/80 px-2 py-1 rounded text-cyber-text border border-cyber-border hover:border-cyber-accent">
  ...
  return (
    <div className="space-y-6 no-print">
      {[1, 2, 3, 4, 5].map((i) => (
        <div
          key={i}
          className="flex flex-col gap-4 p-6 bg-black/20 border border-cyber-border/40 rounded-xl relative justify-between"
        >
