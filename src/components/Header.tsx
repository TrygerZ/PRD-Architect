...
export function Header({ onOpenSettings, onExportMd, onCopy, onPrint, hasData, language, onToggleLanguage }: HeaderProps) {
  return (
      ...
      <div className="flex items-center gap-2 sm:gap-4 ml-auto">
        {hasData && (
          <div className="flex items-center gap-1.5 sm:gap-2 mr-2 sm:mr-4 border-r border-cyber-border pr-2 sm:pr-6">
            <button onClick={onCopy} className="cyber-button text-xs py-1.5 px-2 sm:px-3 flex items-center gap-1" title={language === 'en' ? 'Copy as Text' : 'Salin Text'}>
              <Copy size={14} /> <span className="hidden sm:inline">{language === 'en' ? 'Copy' : 'Salin'}</span>
            </button>
            <button onClick={onExportMd} className="cyber-button text-xs py-1.5 px-2 sm:px-3 flex items-center gap-1" title={language === 'en' ? 'Download Markdown' : 'Unduh Markdown'}>
              <Download size={14} /> <span className="hidden sm:inline">.MD</span>
            </button>
            <button onClick={onPrint} className="cyber-button text-xs py-1.5 px-2 sm:px-3 flex items-center gap-1" title={language === 'en' ? 'Print to PDF' : 'Cetak PDF'}>
        ...
        <button onClick={onOpenSettings} className="p-1.5 sm:p-2 text-cyber-text-dim hover:text-cyber-accent transition-colors" title={language === 'en' ? 'Settings' : 'Pengaturan'}>
          <Settings size={20} />
        </button>
      </div>
    </header>
